...
/**
 * A convenience class for file uploads. Manages files in the upload directory.
 */
public class FileUploadUtilities {

  public static final File UPLOADS_DIR =
      new File(System.getProperty("jboss.server.data.dir"), "uCount_Uploads");
   ...
   * 
   * @param part The Part from the fileInput
   * @param uploadsDirName The uploads directory name. E.x.: "images"
  ...
  /**
   * Uploads an image to a specified path, and crops and/or resizes it to a desired width, using a
   * 1:1 aspect ratio.
   * 
   * @param part The Part from the fileInput
   * @param fileName The new file name of the upload.
   * @param desiredWidth The desired width for the image. (Will also be height; 1:1 aspect ratio)
   * @throws IOException
   */
  public static void uploadImageToPathAndCrop(Part imagePart, String fileName, int desiredWidth)
      throws IOException {
    ...
    if (bufferedImage.getWidth() < bufferedImage.getHeight()) {
      int newSize =
          (int) (bufferedImage.getHeight() * ((double) desiredWidth / bufferedImage.getWidth()));
      ...
      int newSize =
          (int) (bufferedImage.getWidth() * ((double) desiredWidth / bufferedImage.getHeight()));
    ...
    scaledImage =
        cropImage(scaledImage, new Rectangle((scaledImage.getWidth() / 2) - (desiredWidth / 2),
